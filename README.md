...
> Automated Uniswap/Sushiswap arbitrage bot using Balancer flash loans — monitors price discrepancies and executes on-chain trades atomically
...
## How It Works
1. **Monitor** — continuously watches Uniswap V2 and Sushiswap V2 for price discrepancies between token pairs
2. **Flash Loan** — when a profitable spread is detected, borrows tokens from Balancer (zero upfront capital required)
3. **Execute** — swaps on both DEXes atomically, capturing the spread as profit
4. **Repay** — repays the flash loan in the same transaction; profit is sent to your wallet

## Contract Architecture
- `Arbitrage.sol` — core contract; implements `IFlashLoanRecipient`, executes swaps on Uniswap and Sushiswap
- `bot.js` — price monitoring bot; listens for swap events and triggers `executeTrade()`
- `helpers.js` — utilities for price calculation, liquidity checks, slippage, and pair contract interaction

## Security Notes
- Never commit your `.env` file
- Use a dedicated wallet for bot operations — never use your main wallet
- Test thoroughly on a Hardhat fork before mainnet deployment

## About
Built by **Andrew Elston** | [GitHub](https://github.com/BlockchainNooberz) | andrewelston177@gmail.com
